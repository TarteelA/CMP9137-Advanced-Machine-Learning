...
def preprocess_image_data(datapath_source, datapath_target):
    #Full Path File -> Img
    image_data = {}  
    #File Name -> Full Path File
    file_paths = {}  
    labels = []

    #Load Images from Source Folder (Datapath_Source)
    ...
    for i in range(0, len(list_img_ids)):
        ...
        #Save Images in Target Folder (Datapath_Target)
...
#Load Dataset from Target Folder
...
class ImageClassifier(nn.Module):
    def __init__(self, num_classes, input_shape):
        ...
        #Compute Output Size After Convolutions (Dynamically)
    ...
    def _get_conv_output_size(self, shape):
        with torch.no_grad():
            ...
            sample_input = torch.zeros(1, 3, shape[0], shape[1])  
